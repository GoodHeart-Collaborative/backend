...
import * as se from "@modules/selfCare/selfCareConstant";
...
import { selfCareDao } from './selfCareDao';
import * as selfCareConstant from './selfCareConstant'
...
class SelfCareController {
     ...
     */
    async addSelfCare(params: SelfCareRequest.AddSelfCare) {
        try {
            console.log('paramsparamsparamsparams', params);

            const data = await eventDao.insert("selfCare", params, {});


            return selfCareConstant.MESSAGES.SUCCESS.ADDED(data);
    ...
    async selfCareList(params: SelfCareRequest.getSelfCare) {
        try {
            const { pageNo, limit } = params;
            const pipeline = [];
            let match: Object = {};
            match['userId'] = ObjectID(params.userId);
            if (params.categoryId) { match['category._id'] = params.categoryId };
            pipeline.push({
                $match: match
            })
            const data = await selfCareDao.paginate('selfCare', pipeline, pageNo, limit, true);
...
export const selfCareController = new SelfCareController();